...
export default function HistoryPage() {
  ...
  const groupedHistory = historyContext.reduce((acc: Record<string, HistoryItem[]>, item) => {
  ...
  // For showAll=false, only show the most recent N items (across all dates)
  const MAX_ITEMS = 10;
  let displayDates: string[] = sortedDates;
  let displayGrouped: Record<string, HistoryItem[]> = groupedHistory;
  if (!showAll) {
    let count = 0;
    displayDates = [];
    displayGrouped = {};
      ...
      {historyContext.length === 0 ? (
        <Card className="p-6 sm:p-8 text-center bg-zinc-900/80 backdrop-blur">
          <p className="text-zinc-300 text-base sm:text-lg">
            No history found. Start exploring recommendations to build your
            history!
          </p>
        </Card>
      ) : (
        <>
          <div className="space-y-8">
            {displayDates.map((date) => (
              <div key={date}>
                <div className="mb-2 text-zinc-400 text-sm font-semibold uppercase tracking-wider">
                  {date}
                </div>
                ...
                <div className="flex-1 min-w-0 space-y-4 mt-4 md:mt-0">
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-4 text-sm md:text-base text-zinc-300">
                    {selectedItem.data.release_date && (
                      <span>
                        {new Date(selectedItem.data.release_date).getFullYear()}
                      </span>
                    )}
