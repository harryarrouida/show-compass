...
export default function MediaDetails({
  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-[320px_1fr] gap-4 md:gap-8 mb-6 md:mb-12">
      {/* Left Column */}
      <div className="space-y-4 md:space-y-6 bg-zinc-900/50 rounded-lg border border-zinc-800/50 backdrop-blur p-4 md:p-6 w-full">
        {/* Poster with responsive sizing */}
        <div className="relative aspect-[2/3] w-[200px] sm:w-[240px] md:w-[280px] mx-auto rounded-lg overflow-hidden bg-zinc-900/40">
          <Image
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          {details.genres && (
            <div className="overflow-x-auto scrollbar-hide -mx-4 md:mx-0 px-4 md:px-0">
              <div className="flex flex-wrap gap-2 min-w-min justify-center md:justify-start">
                {details.genres.map((genre) => (
                  <span
                    key={genre.id}
                    className="px-3 py-1.5 bg-zinc-800/70 hover:bg-zinc-700/70 rounded-full text-sm text-zinc-200 whitespace-nowrap"
                  >
                    {genre.name}
                  </span>
                ))}
          ...
          {"seasons" in details && details.seasons && (
            <div className="space-y-4 hidden md:block">
              <h3 className="text-sm font-medium text-zinc-300 text-center md:text-left">
                Seasons
              </h3>
      ...
      <div className="space-y-4 md:space-y-6 px-4 md:px-0">
        <div>
          <h1 className="text-2xl md:text-4xl font-semibold text-white mb-3 md:mb-4 text-center md:text-left">
            {details.title}
          </h1>
          <p
            className={`text-base md:text-lg text-zinc-300 leading-relaxed text-center md:text-left bg-zinc-900/50 p-4 md:p-5 rounded-xl border border-zinc-800/50 ${
              showAllOverview ? "" : "line-clamp-1"
          ...
          <button
            ...
            className="mx-auto my-4 text-center w-full text-zinc-400 text-sm font-light hover:text-violet-400 transition-colors duration-300"
          >
