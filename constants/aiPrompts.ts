...
export const generateDefaultPrompt = (mediaDetails: any, type: string) => `
Based on this ${type}:
...
Respond with ONLY a clean JSON object in this exact format:
{
    "recommendations": [
        {
            "title": "Title",
            "reason": "Two clear, concise sentences that explain specific thematic or stylistic connections to ${mediaDetails.title}. Focus on narrative elements, themes, and artistic approach."
        }
    ]
}
...
export const generateCustomPrompt = (details: any, type: string, prompt: string) => `
Based on this ${type}:
Title: "${details?.title}"
